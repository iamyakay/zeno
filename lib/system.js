...
const LISTEN_SCRIPT = [
  ...
  "if ($result -and $result.Text -and $result.Confidence -ge 0.55) { [Console]::WriteLine('FINAL:' + $result.Text) } else { [Console]::WriteLine('LOWCONFIDENCE') }"
].join("; ");
  ...
  return new Promise((resolve) => {
      ...
      for (const line of lines) {
        if (line.startsWith("PARTIAL:")) {
          for (const win of BrowserWindow.getAllWindows()) {
            if (!win.isDestroyed()) win.webContents.send("zeno:listen-partial", line.slice(8));
          }
        } else if (line.startsWith("FINAL:")) {
          finalText = line.slice(6);
        } else if (line.startsWith("LOWCONFIDENCE")) {
          lowConfidence = true;
    ...
    listenChild.on("close", () => {
      ...
      if (text) {
        ...
        finish({ ok: false, error: "I heard something but wasn't sure what. check Settings > System > Sound > Input is set to your real mic, not stereo mix or a virtual device" });
      } else {
...
async function launchAnyApp(name) {
  const clean = String(name || "").replace(/[^\w .+&'-]/g, "").trim();
  if (!clean) return { ok: false, error: "which app?" };
  const builtin = APP_COMMANDS[clean.toLowerCase()];
  if (builtin) {
    const child = spawn(builtin[0], builtin.slice(1), { detached: true, stdio: "ignore", shell: false });
    child.unref();
  ...
  const script = [
    ...
    "if ($direct) { Start-Process $name; Write-Output 'LAUNCHED'; exit }",
    "$apps = Get-StartApps | Where-Object { $_.Name -like \"*$name*\" }",
    "$app = $apps | Sort-Object { $_.Name.Length } | Select-Object -First 1",
    "if ($app) { Start-Process (\"shell:AppsFolder\\\" + $app.AppID); Write-Output ('LAUNCHED:' + $app.Name); exit }",
    "Write-Output 'NOTFOUND'"
  ].join("; ");
  const result = await runPs(script, 15000);
  if (result.output?.startsWith("LAUNCHED")) {
    const launched = result.output.includes(":") ? result.output.split(":").slice(1).join(":") : clean;
    return { ok: true, detail: launched };
  }
  return { ok: false, error: `couldn't find an app called ${clean} on this PC` };
}
