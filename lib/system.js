...
const LISTEN_SCRIPT = [
  ...
  "$result = $rec.Recognize([TimeSpan]::FromSeconds(24))",
  ...
  return new Promise((resolve) => {
    ...
    listenChild.on("close", () => {
      ...
      if (text) {
        ...
        finish({ ok: false, error: "didn't catch that. check the mic is plugged in and Windows speech recognition language matches yours, then try again" });
      }
    });
  });
}
