...
const TINY_URL = "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-tiny.en.bin";
const TINY_SIZE = 77704715;
...
function tinyPath() {
  return path.join(dir(), "ggml-tiny.en.bin");
}

function hasTiny() {
  try {
    return fs.statSync(tinyPath()).size > TINY_SIZE * 0.95;
  } catch {
    return false;
  }
}
...
async function setup(onStatus) {
  ...
  setupPromise = (async () => {
    ...
    if (!hasTiny()) {
      onStatus?.("downloading fast speech model...");
      await download(TINY_URL, tinyPath(), (p) => {
        onStatus?.(`downloading fast speech model... ${Math.round(p * 100)}%`);
      });
    }
    ready = false;
...
function transcribe(wavBuffer) {
  return new Promise((resolve) => {
    ...
    const seconds = Math.max(1, ((wavBuffer.byteLength || wavBuffer.length) - 44) / 2 / 16000);
    const audioCtx = Math.min(1500, Math.ceil(seconds * 50) + 96);
    const useTiny = hasTiny() && seconds <= 12;
    let out = "";
    let child;
    try {
      child = spawn(exePath(), [
        "-m", useTiny ? tinyPath() : modelPath(),
        "-f", wavFile,
        "-l", "en",
        "-t", String(Math.max(2, Math.min(10, require("node:os").cpus().length - 1))),
        "-bs", "1",
        "-bo", "1",
        "-ac", String(audioCtx),
        "-nt",
        ...
        "-nf",
        "--no-prints",
        "--prompt", "ZENO, my desktop assistant. Commands like: open youtube, volume up, close chrome, make a file."
